...
#	os.path.join adds the trailing slash that's silently deleted by abspath
            ...
            inputJpgs = [file for file in natsorted(os.listdir(
                self.inputLocation)) if file.endswith(self.imgExt)]
        ...
        for i in range(0, self.totalImages, 1):
            ...
            if self.aspectRatio >= currentAspectRatio:
                ...
                newImage.paste(resizedImage, ((
                    self.optimalWidth-resizedWidth)//2, (self.optimalHeight-resizedHeight)//2))
            else:
    ...
    def savePdf(self):
        ...
        self.finalPages[0].save(self.outputLocation+self.outputName,
                                save_all=True, append_images=self.finalPages[1:])
...
class Ui(QDialog):
    ...
    def button1(self):
        ...
        fileLocation, _ = QFileDialog.getOpenFileNames(
            self, "Choose files", "", "All Files (*);;Python Files (*.py", options=options)
    #	print(fileLocation)
        ...
        if fileLocation:
            ...
            if len(fileLocation) > 1:
                ...
                if multiplePdfs == False:
                    firstFileLocationPartition = fileLocation[0].rpartition(
                        '/')[2]
                    self.label_8.setText(fileLocation[0].rpartition(
                        '/')[2] + ", " + fileLocation[1].rpartition('/')[2] + ", ...")
